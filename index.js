...
// ตารางปิรามิดประชากร
app.get(rootPart + '/pyramid', cors(corsOptions),timeout('15s'), cache('6 hour'), (req, res) => {
...
app.get(rootPart + '/pyramid60up', cors(corsOptions), timeout('15s'), cache('6 hour'), (req, res) => {
    const personDao = new FFC("person"); // สร้างตัวเข้าถึงฐานข้อมูล ffc ที่ person
    const query = [
        {
            "$match": {
...
// idorg
app.get(rootPart + '/pyramid/:orgId', cors(corsOptions),timeout('15s'), cache('6 hour'), (req, res) => {
    const orgId = req.params.orgId;
...
app.get(rootPart + '/pyramid60up/:orgId', cors(corsOptions), timeout('15s'), cache('6 hour'), (req, res) => {
    const orgId = req.params.orgId;
    console.log(orgId, 'perPersonData');
    const personDao = new FFC("person");
